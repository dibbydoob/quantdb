...
class DbService():
    ...
    def insert_timeseries_df(self, dtype="equity", dformat="spot", dfreq="1d",
        ...
        if meta_series_count == 0:
            doc = {**series_identifier,
                   **{"time_start": series_start, "time_end": series_end, "last_updated": datetime.datetime.utcnow()}}
            ...
            contiguous_series = self._check_contiguous_series(record_start=meta_start, record_end=meta_end,
                                                              new_start=series_start, new_end=series_end)
            ...
            if len(new_head) + len(new_tail) > 0:
                head_records = [{**row.dropna().to_dict(), **{"metadata": series_metadata}} for index, row in
                                new_head.iterrows()]
                tail_records = [{**row.dropna().to_dict(), **{"metadata": series_metadata}} for index, row in
                                new_tail.iterrows()]
            ...
            db_logs.DBLogs().critical(
                "meta series corruption, series count gt 1 insert_timeseries_df {}".format(metalogs))
            exit()
    ...
    async def asyn_insert_timeseries_df(self, dtype="equity", dformat="spot", dfreq="1d",
        ...
        series_start = records[0]["datetime"]
        series_end = records[-1]["datetime"]
        meta_series_count = await (await self._asyn_get_collection_meta(dtype, dformat, dfreq)).count_documents(
            series_identifier)
        ...
        if meta_series_count == 0:
            ...
            meta_record = await (await self._asyn_get_collection_meta(dtype, dformat, dfreq)).find_one(
                series_identifier)
            meta_start = meta_record["time_start"]
            meta_end = meta_record["time_end"]
            contiguous_series = self._check_contiguous_series(record_start=meta_start, record_end=meta_end,
            ...
            if len(new_head) + len(new_tail) > 0:
                ...
                tail_records = [{**row.dropna().to_dict(), **{"metadata": series_metadata}} for index, row in
                                new_tail.iterrows()]
                await (await self._asyn_get_collection(dtype, dformat, dfreq)).insert_many(head_records + tail_records,
                                                                                           ordered=False)
            ...
            db_logs.DBLogs().critical(
                "meta series corruption, series count gt 1 asyn_insert_timeseries_df {}".format(metalogs))
            exit()
    ...
    async def asyn_batch_insert_timeseries_df(self, dtype="equity", dformat="spot", dfreq="1d",
        ...
        with mp.Manager() as manager:
            with mp.Pool(mp.cpu_count()) as pool:
                recordss = pool.map(DbService.unroll_df,
                                    [{"i": i, "sm": series_metadatas[i], "df": dfs[i]} for i in range(len(dfs))])
        ...
        if series_identifiers:
            async for i in (await self._asyn_get_collection_meta(dtype, dformat, dfreq)).find(
                    {"$or": series_identifiers}):
                docs.append(i)
        ...
        for i in range(len(series_identifiers)):
            ...
            if len(matched) == 0:
                ...
                if not contiguous_series:
                    db_logs.DBLogs().critical(
                        "discontiguous series in asyn_batch_insert_timeseries_df {}".format(metalogs[i]))
                    continue
                ...
                db_logs.DBLogs().critical(
                    "meta series corruption, series count gt 1 asyn_batch_insert_timeseries_df {}".format(metalogs[i]))

        with mp.Manager() as manager:
            with mp.Pool(mp.cpu_count()) as pool:
                unrolled_heads = pool.map(DbService.unroll_df,
                                          [{"i": i, "sm": update_identifers[i], "df": new_heads[i]} for i in
                                           range(len(new_heads))])
                unrolled_tails = pool.map(DbService.unroll_df,
                                          [{"i": i, "sm": update_identifers[i], "df": new_tails[i]} for i in
                                           range(len(new_tails))])
        ...
        if new_inserts_seriess:
            await (await self._asyn_get_collection(dtype, dformat, dfreq)).insert_many(new_inserts_seriess,
                                                                                       ordered=False)
        if new_inserts_series_metas:
            await (await self._asyn_get_collection_meta(dtype, dformat, dfreq)).insert_many(new_inserts_series_metas,
                                                                                            ordered=False)
        if new_updates_series_metas:
            await (await self._asyn_get_collection_meta(dtype, dformat, dfreq)).bulk_write(new_updates_series_metas,
                                                                                           ordered=False)

        db_logs.DBLogs().info("successful asyn_batch_insert_timeseries_df {}".format(metalogs))
        return True

    def read_timeseries(self, dtype="equity", dformat="spot", dfreq="1d", period_start=None, period_end=None,
    ...
    async def asyn_read_timeseries(self, dtype="equity", dformat="spot", dfreq="1d", period_start=None, period_end=None,
                                   series_metadata={}, series_identifier={}, metalogs=""):
        self._ensure_coll(dtype=dtype, dformat=dformat, dfreq=dfreq, coll_type="timeseries")
        period_start = max(period_start, datetime.datetime(1970, 2, 1))
        period_end = max(period_end, datetime.datetime(1970, 2, 1))
